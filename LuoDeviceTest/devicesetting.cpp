...
void DeviceSetting::initChart()
{
    ...
    if (m_axisX == nullptr)
    {
        ...
        m_axisX->setRange(0, m_chartdata.XRange);
...
{
    if (m_point_data.size() > freq * m_chartdata.XRange)
    {
...
void DeviceSetting::on_draw_switch_triggered()
{
    ...
    {
        ui->draw_switch->setText("绘图:关");
        m_updateTimer.stop();
    }
}

void DeviceSetting::on_cmd_switch_triggered()
{
...
void DeviceSetting::on_chart_setting_triggered()
{
    m_chartsetting->SetCurrentChartData(m_chartdata);
    m_chartsetting->show();
}
...
void DeviceSetting::on_m_comboBox_ChannelSelect_currentIndexChanged(int index)
{
