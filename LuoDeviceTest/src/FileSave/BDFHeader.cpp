...
int BDFHeader::InitOutBDFHeader(const std::string& file_path)
{
	...
	auto _formattime = [](int _data) {std::string _tmp_str = std::to_string(_data);	return std::string(2 - _tmp_str.length(), '0') + _tmp_str; };
...
const int BDFHeader::GetNumberOfDataRecord()
{
	assert(m_bdf_header != nullptr);
	char _tmp[9] = { 0 };
